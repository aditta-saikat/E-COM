...
const signToken = (overrides = {}) => jwt.sign(
  { sub: '000000000000000000000001', email: 'test@example.com', role: 'customer', ...overrides },
...
test('requesting a shop rejects requests without a Bearer token', async (t) => {
  const server = app.listen(0);
  t.after(() => server.close());

  const response = await request(server, '/api/shops', {
    method: 'POST',
    body: { name: 'Test Shop' },
  });
...
test('creating a product rejects a customer-role token', async (t) => {
  const server = app.listen(0);
  t.after(() => server.close());
  ...
  const response = await request(server, '/api/products', {
    method: 'POST',
    body: { name: 'Test product', price: 10, sku: 'TEST-1' },
    token: signToken({ role: 'customer' }),
  });

  assert.equal(response.statusCode, 403);
  assert.equal(response.body.error, 'You do not have permission to access this resource');
});

test('the admin-only pending shops list rejects a shop_admin-role token', async (t) => {
  const server = app.listen(0);
  t.after(() => server.close());

  const response = await request(server, '/api/shops/pending', {
    token: signToken({ role: 'shop_admin' }),
  });
